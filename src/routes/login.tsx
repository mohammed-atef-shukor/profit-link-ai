...
function LoginPage() {
  ...
  const { currentUser, loading, role, roleLoading, login, googleLogin } = useAuth();
  ...
  useEffect(() => {
    if (!loading && !roleLoading && currentUser && role !== null) {
      navigate({ to: roleHomePath(role), replace: true });
    }
  }, [loading, roleLoading, currentUser, role, navigate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(getFirebaseErrorMessage(e, "Sign-in failed"));
    } finally {
      setSubmitting(false);
    }
  };
...
export function Field({
  ...
  return (
    <div>
      ...
      <div
        className={`mt-1.5 flex items-center gap-2 rounded-xl border bg-surface px-3.5 py-3 transition-colors ${error ? "border-destructive/60" : "border-border focus-within:border-primary/60"}`}
      >
...
export function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" className="size-4" aria-hidden>
      ...
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.07.56 4.21 1.65l3.16-3.16C17.45 2.1 14.97 1 12 1A11 11 0 0 0 2.18 7.04l3.66 2.84C6.71 7.3 9.14 5.38 12 5.38Z"
      />
    </svg>
  );
}
